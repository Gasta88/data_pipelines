...
class LoadDimensionOperator(BaseOperator):
    """A class to represent the Airflow custom operator to load dimensions on AWS Redshift."""
    ui_color = '#80BD9E'
    ...
    def execute(self, context):
        ...
        if not self.append_mode:
            logging.info("Clearing data from destination dimension table")
        ...
        formatted_sql = LoadDimensionOperator.insert_sql.format(
            ...
            formatted_columns,
            self.sql_select)
        logging.info(f"Load {self.table} dim via SQL: {formatted_sql}")
